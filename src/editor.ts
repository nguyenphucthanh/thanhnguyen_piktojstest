...
class CanvasEditor {
	...
	public listImage: HTMLUListElement;
	public formUpload: HTMLFormElement;
	public canvasObjects: ICanvasObject[] = [];
	public selectedCanvasObject: string = null;

	constructor() {
		this.listImage = document.getElementById('listImage') as HTMLUListElement;
		this.canvas = document.getElementById('canvas') as HTMLDivElement;
		this.formUpload = document.getElementById('formUpload') as HTMLFormElement;
	...
	bindEventsToCanvasArea() {
		...
		this.canvas.onmousemove = (event) => {
			const div = this.canvas.querySelector('.moving') as HTMLElement;
			if (div) {
	...
	formUploadSubmit(event) {
		...
		const fileUploadInput = this.formUpload.querySelector('#fileUpload') as HTMLInputElement;
	 ...
	 * @param reposition
	 */
	addObjectToCanvas(obj: ICanvasObject, reposition = true) {
	...
	createDivWrapper(obj: ICanvasObject) {
	 ...
	 * @param obj
	 */
	addPictureToCanvas(obj: ICanvasObject): HTMLElement {
		// make a div to wrap main object
	 ...
	 * @param obj
	 */
	addTextToCanvas(obj: ICanvasObject): HTMLElement {
		// make a div to wrap main object
		const div = this.createDivWrapper(obj);
	 ...
	 * @param div
	 */
	bindEventToCanvasObject(div: HTMLElement) {
		...
		for (let i = 0; i < div.children.length; i++) {
			const childEle = div.children.item(i) as HTMLElement;
	 ...
	 */
	calculateCenterPosition(div: HTMLElement): IPosition {
		// get canvas size
